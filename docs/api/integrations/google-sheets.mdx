...
:::info
Before using the extension, you need to set your API key. Click on the `Muse` tag in the toolbar and select `Settings` > `Register API Key`.
:::

> You can load an example sheet from the `Muse` tab to start experimenting.

A basic sheet layout for this extension:

| `Prompt` (this is mandatory) | `n_tokens` | `seed`  | (more params) | `Completion`           |
| ---------------------------- | ---------- | ------- | ------------- | ---------------------- |
| TODO: Find a prompt          | `15`       | `67`    |               | TODO: write completion |
| TODO                         | `1`        | `34885` |               | TODO                   |

:::info
To complete cells, select an area with the `prompt` keyword in the upper-left cell.
...
-   `stop_words` <ParamType type="Array<string>" /> <ParamDefault default="null" />
