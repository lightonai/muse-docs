...
will just take a few minutes of your time to go through this page — and you can follow-up with one of our in-depth 📚 **[Guides](/home/english/prompt)**.
...
We abstract away all the fancy machine learning, and give you **a simple interface to ✍️ [Create](/api/endpoints/create),
🔬️ [Evaluate](/api/endpoints/select), 📊 [Represent](/api/endpoints/embed), and **⚙️ [Process](/api/endpoints/process)** language** (upcoming).
We believe that all language tasks can be tackled through these simple primitives:

-   **Use ✍️ [Create](/api/endpoints/create) to generate text** according to natural language instructions
    (e.g. copywriting, conversational AI, etc.).
-   **Use 🔬️ [Evaluate](/api/endpoints/select) to understand text**, and evaluate the likelihood of different options
    (e.g. classification, automated Q&A, etc.).
-   **Use 📊 [Represent](/api/endpoints/embed) <Alpha /> to build rich representations of text**, to directly compare samples or use in a downstream pipeline (e.g. semantic search, document clustering, etc.)

These features can be combined to **⚙️ [Process](/api/endpoints/process) text, achieving complex end-results**
...
**The prompt is the input text you submit to the model in the ✍️ [Create](/api/endpoints/create) endpoint**.
...
examples can also be helpful. To learn more, read our guide about **📜 [Prompt design](/home/english/prompt)**.
...
**🔬️ [Evaluate](/api/endpoints/analyse) endpoints rely on likelihood to understand text**. You can access the log-probabilities
associated with words in a sentence using 🧪 **[Analyse](/api/endpoints/analyse)**, to use in a downstream pipeline, or you
can directly use 🔘 **[Select](/api/endpoints/select)** to perform likelihood-based text classification. See our guide
about 🥇 **[reviews classification](/home/english/review-classification)** with the Muse API to learn more.

**Likelihood can be manually manipulated in ✍️ [Create](/api/endpoints/create) to steer generation**. Use `word_biases` to manually
...
on 🎛️ **[Steering generation for SEO](/home/english/seo)** for more.
...
**📊 [Represent](/api/endpoints/embed) endpoints directly expose the final embeddings built by the model**. You can
leverage these embeddings in your machine learning pipeline (applying clustering, t-SNE, or a classifier of your choice
on them). You can also use ⚖️ **[Compare](/api/endpoints/compare)** to directly compare different texts and select the most similar based on embeddings.
...
**On occasion, you have to be mindful of tokens.** For instance, in ✍️ **[Create](/api/endpoints/create)**, the model can only
...
distribution can then be _sampled_ to generate text with ✍️ **[Create](/api/endpoints/create)**. We expose three modes of
generation:
